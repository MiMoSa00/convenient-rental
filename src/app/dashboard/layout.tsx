  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100">
      ...
      <header className="sticky top-0 z-40 bg-white/95 dark:bg-gray-900/95 backdrop-blur supports-[backdrop-filter]:bg-white/60 supports-[backdrop-filter]:dark:bg-gray-900/60 border-b border-gray-200 dark:border-gray-800 transition-all duration-200">
        {/* Add slight top padding for breathing room; keep fixed height for sidebar offset correctness */}
        <div className="h-16 pt-1 sm:pt-1 lg:pt-2 flex items-center justify-between px-3 sm:px-4 lg:px-8 max-w-7xl mx-auto text-gray-900 dark:text-white">
          <div className="flex items-center space-x-3 sm:space-x-4 min-w-0">
            ...
            {/* Brand: logo with responsive sizing; tagline responsive and non-breaking */}
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              ...
              >
                <Image
                  ...
                  height={48}
                  priority
                  className="w-9 h-9 sm:w-10 sm:h-10 lg:w-12 lg:h-12 rounded-md object-contain shrink-0 mt-0.5 sm:mt-0 lg:mt-1"
                />
              </Link>
              <p className="ml-1 sm:ml-2 text-gray-700 dark:text-white leading-snug whitespace-nowrap overflow-hidden text-ellipsis text-xs sm:text-sm lg:text-base max-w-[8rem] sm:max-w-[16rem] lg:max-w-[26rem]">
          ...
          <div className="flex items-center space-x-3 sm:space-x-4 lg:space-x-6">
            ...
            >
              {theme === "light" ? (
                <Moon className="h-5 w-5 sm:h-5 sm:w-5 lg:h-6 lg:w-6" />
              ) : (
                <Sun className="h-5 w-5 sm:h-5 sm:w-5 lg:h-6 lg:w-6" />
              )}
            </button>
            ...
            <div className="relative">
              ...
              >
                ...
                {unreadCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 h-4 w-4 bg-red-500 text-white text-[10px] sm:text-xs rounded-full flex items-center justify-center font-medium">
            ...
            <div className="flex items-center space-x-2">
              ...
              {/* Hide on very small screens; never wrap and truncate on larger screens */}
              <span className="hidden sm:inline font-medium text-sm md:text-base whitespace-nowrap truncate max-w-[6rem] md:max-w-[10rem] lg:max-w-[16rem]">
                {userName}
              </span>
              {session && (
                <button
                  onClick={() => signOut({ callbackUrl: "/" })}
                  className="ml-2 sm:ml-3 bg-red-500 text-white whitespace-nowrap text-xs sm:text-sm md:text-base px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-lg hover:bg-red-600 transition-colors"
                >
                  Log Out
                </button>
              )}
